...
struct MacPairingScannerView: View {
    let onScan: (MacBridgePairingPayload) -> Void
    ...
    var body: some View {
        ZStack {
            ...
            if isCheckingPermission {
                ...
                MacPairingCameraPreview { code, resetScanLock in
        ...
        .alert("Paste Pairing Payload", isPresented: $isShowingManualEntry) {
            ...
            Text("Paste the full pairing payload JSON from the Mac bridge if scanning is unavailable.")
        }
    }
    ...
    private func handleManualPayload() {
        switch validateMacPairingPayload(manualPayload) {
        case .success(let payload):
...
private struct MacPairingCameraPreview: UIViewRepresentable {
    let onScan: (String, _ resetScanLock: @escaping () -> Void) -> Void

    func makeUIView(context: Context) -> MacPairingCameraView {
        let view = MacPairingCameraView()
    ...
    func updateUIView(_ uiView: MacPairingCameraView, context: Context) {}

    static func dismantleUIView(_ uiView: MacPairingCameraView, coordinator: ()) {
        uiView.stopCamera()
    }
}

private final class MacPairingCameraView: UIView, AVCaptureMetadataOutputObjectsDelegate {
